...
data_params = {'id': 'gene_expression', 'type': 'prostate_paper', 'params': {'data_type': 'gene_expression', 'drop_AR': False}}
data = Data(**data_params)
x_meth, y_meth, info_meth, columns_meth = data.get_data()
df_meth = pd.DataFrame(x_meth, index=info_meth, columns=list(columns_meth))
df_meth['y'] = y_meth
...
def plot_stacked_hist_mut(df, gene_name, ax):
    ...
    bars = summary.T.plot.bar(stacked=True, ax=ax)
    # bars = summary.T.plot.bar(stacked=True, ax=ax, color=color )
...
def plot_stacked_hist_meth(df, gene_name, ax):
    ind = np.sort(df[gene_name].unique())
    print(ind)
    primary_df = df[df['y'] == 0]
    mets_df = df[df['y'] == 1]
    ...
    # ax.set_title(gene_name, fontdict=dict(family='Arial', weight='bold', fontsize=12))
    ax.set_title('Number of methylated samples', fontdict=dict(family='Arial', weight='normal', fontsize=10))    


#selected_genes = ['AR', 'TP53', 'PTEN', 'FGFR1', 'MDM4', 'RB1', 'NOTCH1', 'MAML3', 'PDGFA', 'EIF3E']
selected_genes = ['RAE1', 'TP53', 'PTEN', 'UBE2C', 'CCT5', 'NUP107', 'CEP72', 'MRPL36', 'CD40', 'BLM', 'BRCA1', 'BRCA2', 'PALB2', 'RBBP8', 'RAD21', 'MYC', 'RB1', 'VAV1', "RAD21", "NUP153", "NUP155", "RUNX1", "RFC4", "NCKAP1L", "ATM","BARD1", "BRIP1", "CHEK1", "CHEK2", "FAM175A", "MRE11A", "NBN", "RAD51C", "APC", "ALOX5", "BRD7", "USP44", "STAG3", "EZH2"]
...
def run():
    for g in selected_genes:
        # fig =plt.figure()
        fig, axes = plt.subplots(nrows=1, ncols=3, sharey=True, figsize=(8, 6), dpi=200)
        plt.ylim(0, 110)
        ...
        if g in df_meth.columns:
            # plot_hist_mut(df_mut, g, axes[1])
            plot_stacked_hist_meth(df_meth, g, axes[2])
