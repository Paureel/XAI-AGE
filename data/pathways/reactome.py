...
def complete_network(G, n_leveles=4):
    ...
    terminal_nodes = [n for n, d in sub_graph.out_degree().items() if d == 0]
...
class ReactomeNetwork():
	...
	import networkx
        ...
        human_hierarchy = [tuple(x) for x in human_hierarchy.values]
        
        #net = networkx.from_pandas_datatframe(human_hierarchy, 'child', 'parent', create_using=nx.DiGraph())
        net = networkx.from_edgelist(human_hierarchy, create_using=nx.DiGraph())
        #net = nx.from_pandas_edgelist(human_hierarchy, 'child', 'parent', create_using=nx.DiGraph())
        net.name = 'reactome'
        print("hello")
	print(net.in_degree().itervalues())
        # add root node
        roots = [n for n, d in net.in_degree().items() if d == 0]
        root_node = 'root'
    ...
    def get_layers(self, n_levels, direction='root_to_leaf'):
        ...
        terminal_nodes = [n for n, d in net.out_degree().items() if d == 0]  # set of terminal pathways
